...
欢迎来到现代密码学实践之 DES 穷举攻击！在本次实践中，你将会使用 Python 来尝试穷举一个密文的密钥并将其解密（Part 1）！同时，我们将会使用 Python 来实现一个简单的 DES 加密算法（Part 2）！在[这里](#)获取这次实践将会用到的所有初始代码！国内一行命令下载：

```bash
# Linux
wget https://path/to/attack-project -o DES-Attack
# Windows（待补充，希望有同学测试后能在评论区给出命令
```
